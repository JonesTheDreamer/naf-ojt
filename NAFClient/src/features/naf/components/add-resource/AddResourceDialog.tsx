...
import { Globe, Mail, FolderOpen, Settings2, Zap, Plus, CheckCircle2 } from "lucide-react";
...
const TAB_META: Record<TabId, { label: string; icon: typeof Globe; color: string }> = {
...
export function AddResourceDialog({ naf, open, onOpenChange }: AddResourceDialogProps) {
  ...
  const [specialResources, setSpecialResources]   = useState<BasicResourceWithDate[]>([]);
  ...
  const availableBasic = (getAllResource.data ?? []).filter(
    (r) => !r.isSpecial && !existingResourceIds.includes(r.id) && !blockedByGroup.has(r.id),
  );
  const availableSpecial = (getAllResource.data ?? []).filter(
    (r) => r.isSpecial && !DEDICATED_SPECIAL_IDS.includes(r.id) && !existingResourceIds.includes(r.id) && !blockedByGroup.has(r.id),
  );

  const newEntry = () => crypto.randomUUID();

  const addInternetEntry = () =>
    setInternetEntries((prev) => [...prev, {
      _id: newEntry(), internetPurposeId: null, internetResourceId: null,
      purpose: "", dateNeeded: "", isOther: false, newPurposeName: "",
      newPurposeDescription: "", newResourceName: "", newResourceUrl: "", newResourceDescription: "",
    }]);
  ...
  const patchSharedFolderEntry = (_id: string, patch: Partial<SharedFolderEntry>) => setSharedFolderEntries((p) => p.map((e) => e._id === _id ? { ...e, ...patch } : e));
  ...
  const allComplete = allSpecialHavePurpose && internetEntries.every(isInternetEntryComplete) && groupEmailEntries.every(isGroupEmailEntryComplete) && sharedFolderEntries.every(isSharedFolderEntryComplete);
  const canSubmit = hasAnything && allComplete && !isSubmitting;

  const reset = () => {
    setBasicResources([]); setSpecialResources([]);
    setInternetEntries([]); setGroupEmailEntries([]); setSharedFolderEntries([]);
    setSubmitErrors([]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitErrors([]);
    const result = await submit({ nafId: naf.id, basicResources, specialResources, internetEntries, groupEmailEntries, sharedFolderEntries });
  ...
  const visibleTabs: TabId[] = [
    ...
    "internet", "email", "folder",
  ] as TabId[];

  const totalSelected = counts.basic + counts.special + counts.internet + counts.email + counts.folder;
      ...
      <DialogContent className="w-full max-w-xl p-0 gap-0 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-white border-b px-5 pt-5 pb-0">
          <div className="mb-4 pr-8">
            <h2 className="text-slate-900 font-bold text-lg leading-tight">Add Resources</h2>
            <p className="text-slate-400 text-xs mt-0.5 font-mono">{naf.reference}</p>
          </div>
        ...
        {/* Tab content */}
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden bg-slate-50/60">
              ...
              <div className="space-y-3">
                {internetEntries.length === 0 ? (
                  ...
                  <>
                    ...
                    <AddEntryButton onClick={addInternetEntry} label="Add another entry" />
                  </>
                )}
              </div>
            )}
              ...
              <div className="space-y-3">
                {groupEmailEntries.length === 0 ? (
                  ...
                  <>
                    ...
                    <AddEntryButton onClick={addGroupEmailEntry} label="Add another entry" />
                  </>
                )}
              </div>
            )}
              ...
              <div className="space-y-3">
                {sharedFolderEntries.length === 0 ? (
                  ...
                  <>
                    ...
                    <AddEntryButton onClick={addSharedFolderEntry} label="Add another entry" />
                  </>
                )}
              </div>
            )}
              ...
              <div className="flex items-center gap-2">
                {totalSelected > 0 && (
                  <div className="flex items-center gap-1.5 text-xs text-slate-500">
                    ...
                    <span><span className="font-semibold text-slate-700">{totalSelected}</span> resource{totalSelected !== 1 ? "s" : ""} selected</span>
                  </div>
                )}
              </div>
              <div className="flex items-center gap-2">
...
function EmptyTabState({
  icon: Icon, label, action, onAdd,
}: { icon: typeof Globe; label: string; action: string; onAdd: () => void }) {
  return (
